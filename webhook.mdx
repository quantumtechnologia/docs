...
Toda notificação que enviamos para o seu endpoint é assinada. Fazemos isso incluindo um header com o nome `Quantumpay-Signature` em cada evento que enviamos. Isso permite verificar e garantir que o evento foi enviado pelo Quantum Pay e não por um terceiro.

### Formato do Header Quantumpay-Signature

O header `Quantumpay-Signature` contém um **timestamp** e uma ou mais **assinaturas**. O timestamp é prefixado por `t=` e cada assinatura é prefixada por um schema. Schemas começam com `v` seguido de um integer. Atualmente existe apenas um schema de assinatura que é o `v1`.

**Exemplo do Header Quantumpay-Signature:**

```http
Quantumpay-Signature: t=1580306324381,v1=5257a869e7ecebeda32affa62cdca3fa51cad7e77a0e56ff536d0ce8e108d8bd
```
...
function verifyWebhookSignature(requestHeaders, requestPayload, secretKey) {
  ...
  const signatureHeader = requestHeaders['quantumpay-signature']
  
  if (!signatureHeader) {
    throw new Error('Header Quantumpay-Signature não encontrado')
  }
...
def verify_webhook_signature(request_headers, request_payload, secret_key):
    ...
    """
    # Passo 1: Extrair timestamp e assinatura do header
    signature_header = request_headers.get('quantumpay-signature')
    
    if not signature_header:
        raise ValueError('Header Quantumpay-Signature não encontrado')
...
function verifyWebhookSignature($requestHeaders, $requestPayload, $secretKey) {
    // Passo 1: Extrair timestamp e assinatura do header
    $signatureHeader = $requestHeaders['quantumpay-signature'] ?? $requestHeaders['Quantumpay-Signature'] ?? null;
    
    if (!$signatureHeader) {
        throw new Exception('Header Quantumpay-Signature não encontrado');
    }
...
const requestHeaders = {
  'Quantumpay-Signature': 't=1580306991086,v1=348a92ec7864e30fc9cf3ea91b2e6e1392a14c8379103cb1d8e48e39334a4fd8'
}

// Extrair o valor de 't' do Header 'Quantumpay-Signature'
const reqTimestamp = requestHeaders['Quantumpay-Signature'].split(',')[0].split('=')[1]
// '1580306991086'

// Extrair o valor de 'v1' do Header 'Quantumpay-Signature'
const reqSignature = requestHeaders['Quantumpay-Signature'].split(',')[1].split('=')[1]
